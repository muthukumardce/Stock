...
export async function createApp(options={}){
  ...
  try{
    ...
    app.get('/api/state',(req,res)=>{
      ...
      const floor=store.event_floor();
      res.json({state:snapshot(),events,event_floor:floor,event_cursor:Math.max(floor,events.at(-1)?.id??Number(after??0)),equity_history:store.samples(cfg.trading_mode)});
    });
    ...
    app.delete('/api/events',(req,res)=>{
      ...
      if(!plainObject(req.body)||Object.keys(req.body).length)throw failure(422,'Clear All takes no filters; it removes all saved activity entries.');
      res.json({ok:true,...store.clear_events()});
    });
    ...
    app.put('/api/settings',async(req,res)=>{
      ...
      if(!['selected','all','ignore'].includes(values.manage_existing_holdings)||!Array.isArray(values.managed_symbols)||values.managed_symbols.length>500||values.managed_symbols.some(s=>typeof s!=='string'||!/^[A-Z0-9&.-]{1,40}$/.test(s)))throw failure(422,'Open Settings → Existing holdings. Use NSE trading symbols separated by commas, with at most 500 symbols.');
      if(typeof values.intraday_enabled!=='boolean'||typeof values.swing_enabled!=='boolean'||!['intraday','swing'].every(s=>Number.isFinite(values[s+'_allocation_pct'])&&values[s+'_allocation_pct']>=0&&values[s+'_allocation_pct']<=1&&(!values[s+'_enabled']||values[s+'_allocation_pct']>0))||values.intraday_allocation_pct+values.swing_allocation_pct>1.000000001)throw failure(422,'Open Settings → Trading strategies. Allocations must total at most 100% of available trading funds; each enabled strategy needs a positive allocation.');
      await state.control.run(async()=>{const view=engine.snapshot();if(state.restartRequired||view.status==='running'||view.positions?.length||view.pending_orders?.length)throw failure(409,'Pause entries in Overview, then resolve managed positions and pending orders in Orders & trades before changing Settings. Apply any pending server restart first.');values.managed_symbols=[...new Set(values.managed_symbols)].sort();store.set('strategy_settings',values);store.event('settings.changed','Trading strategies and holding permissions updated.',values);});res.json({ok:true,settings:values});
    });
    ...
    app.put('/api/config',async(req,res)=>{
      ...
      await state.control.run(async()=>{const view=engine.snapshot();if(state.restartRequired)throw failure(409,'Restart the server before saving further application settings');if(view.status==='running'||view.positions?.length||view.pending_orders?.length||view.unmanaged_live_exposure)throw failure(409,'Pause entries in Overview, then resolve managed positions and pending orders in Orders & trades before changing application settings.');
