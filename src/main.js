...
export async function createApp(options={}){
  ...
  try{
    ...
    app.put('/api/settings',async(req,res)=>{
      ...
      await state.control.run(async()=>{const blocker=settingsBlocker(engine.snapshot(),state.restartRequired);if(blocker)throw failure(409,blocker.message);values.managed_symbols=[...new Set(values.managed_symbols)].sort();store.set('strategy_settings',values);store.event('settings.changed','Trading strategies and holding permissions updated.',values);});res.json({ok:true,settings:values});
    });
    ...
    app.put('/api/config',async(req,res)=>{
      ...
      await state.control.run(async()=>{const blocker=settingsBlocker(engine.snapshot(),state.restartRequired);if(blocker)throw failure(409,blocker.message);
