...
export class Settings {
  ...
  validate() {
    ...
    for(const [key,min,max] of [['candidate_wait_ms',0,10000],['min_market_samples',1,9000],['max_trades_per_day',1,100],['loss_streak_limit',1,20],['loss_cooldown_minutes',1,240],['research_symbols',1,50],['research_days',10,60]])if(!Number.isInteger(this[key])||this[key]<min||this[key]>max)throw new Error(`${key} must be an integer between ${min} and ${max}`);
