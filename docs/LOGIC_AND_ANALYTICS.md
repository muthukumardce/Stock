...
This guide covers strategy version **3.1.1**. For installation, environment variables, Cloudflare Tunnel and Zerodha configuration, see the [README](../README.md). The [strategy reference](STRATEGIES.md) specifies all 11 setup families, their thresholds, direction rules and scores; the [candle catalogue](CANDLE_PATTERNS.md) defines the 42 recognized formations.
...
8. Automatic historical research compares baseline and enhanced rules on a sample or all eligible Nifty Total Market members. It never refines parameters, changes trading settings or starts orders.
...
| Market breadth | Disabled | Informational by default. When enabled: at least 30 liquid stocks, 20% fresh universe coverage and 45% advancing shares for longs or declining shares for shorts. |
...
Cash-derived capital and these percentages determine the rupee budgets used by the formulas below. Strategy and existing-holding settings apply without a restart, after entries are paused and managed live positions/pending orders are resolved. Paused simulated positions and their results are retained and do not block settings changes. Saved live exposure from another mode still blocks changes. Application settings such as mode, risk, worker capacity, port and data directory require the same checks followed by a server restart. State polling and both save endpoints use the same specific blocker message, including the affected symbols where available. Administrator password/key changes revoke dashboard sessions without changing trading permission.
...
Historical warmup prioritizes managed positions, then stocks never warmed during this session, then refreshes of previously warmed stocks. Current turnover ranks stocks within each group. It loads completed current-session bars and, for enhanced analysis, a separate prior-session seed. The loader searches the preceding seven calendar days and caches valid seeds for today's connection. Strategy validation checks each supplied session for increasing, uninterrupted five-minute timestamps. Neither the forming candle nor a fabricated missing interval is used.

Cached and in-memory prior seeds must pass the same loader checks as new history: at least 34 valid consecutive candles from one earlier session, no more than seven calendar days old, ending at 15:25; alternatively, exactly 72 consecutive candles spanning 09:15–15:10 for sessions on or after 3 August 2026. The latter is an indicator seed only and does not supply an auction closing price for gap setups. A malformed, stale, current-day, future or other incomplete seed is not reused to mark warmup complete; invalid in-memory completion state is cleared and the prior history range is requested again. A valid seed keeps the normal current-session-only download path.
...
- Remaining target reward divided by planned stop risk, both after estimated costs, meets `min_entry_reward_risk` (default 1.5, configurable from 1 to 10). The check uses the submitted entry limit after spread, entry slippage and tick rounding, and the rounded protective stop. It never extends the target or tightens the stop to make a trade qualify.
- Enough capital, risk budget, cash and visible quantity at the applicable top-of-book price are available.
- When the optional market-breadth gate is enabled, its live sample is ready and supports the trade direction. Daily activity and loss-cooldown limits permit new risk.
...
For entry price `E`, direction `d` (+1 long, -1 short), fee fraction `f` and adverse exit slippage `s`, the planned exits are `T = target * (1 - d*s)` and `S = stop * (1 - d*s)`. Net reward per share is `d*(T-E) - f*(E+T)`; net stop risk is `d*(E-S) + f*(E+S)`. Both must be positive and their ratio must meet the configured minimum. Entry already includes entry-side execution adjustments, so they are not charged twice. Execution uses the existing estimated `f=0.001` and `s=0.0005`; research uses its explicit fee/slippage settings at the next observed open. This is planned economics, not a prediction of the fill or a guaranteed loss limit: gaps and execution failures can exceed estimates. A limit fill may improve on the submitted entry. Rejections expose the ratio in Latest analysis and count toward scan summaries; accepted paper fills, cover intents and research trades retain the entry calculation. Existing positions retain their exit rules.
...
Confirmed rejections retain the exception type, HTTP status and a fixed, actionable explanation in the intent, Activity log and signal decision. Recognized causes include IP restrictions, expired sessions, unavailable cover orders, insufficient margin and invalid stop triggers. Raw broker text is not published because it may contain credentials or account data. An unrecognized HTTP 403 `PermissionException` advises checking trading permissions and the outbound static-IP whitelist without claiming either is the confirmed cause. Rejections before acknowledgement may have no order-book entry in Kite.

**Market breadth** remains visible for reference and its entry gate is disabled by default. It uses eligible subscribed stocks' quotes received within ten seconds. Fresh coverage is fresh eligible quotes divided by eligible subscribed instruments; the breadth sample further requires positive open/last prices and the configured turnover estimate. Advancing means latest price is above today's open, not yesterday's close. The denominator is advancing + declining + unchanged eligible shares. If explicitly enabled, the default thresholds require at least 30 liquid stocks and 20% fresh coverage, with at least 45% advancing for longs or 45% declining for shorts. While disabled, neither missing breadth coverage nor insufficient directional participation blocks entries. Other entry and exit checks still apply.

**Market context and announced events** come from read-only NSE/NSE Indices sources and broker index data. The app loads NIFTY Total Market/500/50 and selected sector constituents (Bank, IT, Pharma, Auto, FMCG and Metal), board-meeting and event-calendar announcements, and available benchmark candles. It validates schemas and labels freshness/coverage; it does not infer missing industry membership from a symbol name. The default limits are seven days for classifications, 60 minutes for event-source observations and 120 seconds for benchmark quotes. Current verified membership can select sector context for strategy/research joins.
...
The dashboard's Decision controls explains current breadth, estimated exposure, ranked candidates and global blockers. Candidate-specific decisions remain in Latest analysis; routine blocked decisions are counted in activity summaries. Broker order rejections remain individual events. Defaults and implementation: [config.js](../src/config.js), [decision-controls.js](../src/decision-controls.js), [trading.js](../src/trading.js).
...
Sizing rounds down to whole shares. Nonfinite values, invalid prices, nonpositive budget or quantity below one prevent the order. The broker adapter also rejects fractional, unsafe or nonpositive quantities before transmitting new orders, quantity modifications or GTT legs; price-only modifications remain supported. The extra 0.2% in per-share risk budgets estimated entry and exit costs; the 0.1% in entry notional budgets entry costs.
...
In Settings, choose selected symbols, all existing NSE holdings, or **Ignore stocks I already own**. The default selection is empty. Ignore overrides saved selections, disables holding adoption and exits, and blocks new intraday long/short and swing entries for currently held symbols in both paper and live mode. Settled, unsettled and pledged quantities, plus positive CNC positions not yet reflected in holdings, count as owned. Ignored holdings remain in account exposure calculations. Changing the policy requires paused entries with no managed live exposure or pending orders; it cannot abandon existing protection. Simulated positions are retained. Live adoption happens only while trading is running. Previously adopted holdings and existing live swing positions continue exit monitoring when new entries are paused.
...
The activity log records order state changes and execution decisions, executed trades, position/holding changes, holdings authorization requests/checks, sampled balance changes, connection changes, settings changes, paper fills and errors. Routine scanner candidates, blocked entry decisions and candle rejection counts are grouped in approximately five-minute summaries; individual recent signals remain in Latest analysis. History coverage summaries record changed counts at most once per five minutes plus completion, while Background retains live progress. Authorization events contain symbols, quantities and outcomes, never TPIN or OTP. It does **not** persist every raw market tick or every rejected candle as a separate event. Balances are sampled at most once per minute; equity is sampled every 30 seconds while connected.
...
Activity errors and common scanner blockers display fixed troubleshooting instructions with links to relevant settings or official broker pages. Help is generated from event types, diagnostic codes and recognized conditions on read, including older records. Raw broker error text is never copied into the instructions. Active engine errors display the same guidance beside the dashboard notice.

Clear All uses an authenticated, CSRF-protected `DELETE /api/events` request with no filters. It atomically deletes saved event rows and persists the highest allocated event ID as a clearing watermark; SQLite’s event sequence is not reset. Trading/recovery journals, sessions, settings and other tables are untouched. Polling and streaming carry the watermark, so old in-flight responses cannot repopulate cleared entries and all open dashboards converge on the cleared history. New events retain increasing IDs. Export includes only entries remaining after the clear.
...
| `market_breadth_unavailable`, `market_breadth_defensive` | Only when the optional breadth gate is enabled: insufficient live coverage or too few eligible shares moving in the trade direction. |
...
The Research page runs only baseline and enhanced comparisons in isolated CPU workers. Automatic research starts after connection when data and resources are ready; reports are cached by current date, strategy version and relevant research/settings signature. Run analysis requests another comparison. Cancellation or failure retains the previous completed report, which also survives restarts. Workers receive candles and numeric settings, never broker credentials or an order client. Research cannot change strategy thresholds, execution mode or permissions, or arm paused trading. Retired queued parameter applications are deleted on service initialization; saved thresholds remain unchanged. Old comparison reports remain readable with their refinement section omitted.

### Dataset and scope

The source universe is the official current **Nifty Total Market** constituent list, approximately 750 stocks. `research_symbols` retains the saved count (20 by default); `0` selects all eligible constituents. Positive counts up to 1,000 choose a reproducible sample by cycling across official industry groups with a stable hash order within each group, without reading prices or returns. A count above available membership selects only those available; outside stocks never fill missing places. Stale or unavailable membership blocks collection. The list is validated, cached and refreshed by Market context, with corporate-action placeholders excluded. The report records the source, observation time, member count, eligible count, selected count and excluded members. Current membership retains survivorship bias and is not historical membership reconstruction.

Intraday collection asks for the prior 45 calendar days of five-minute candles by default, excluding today. Settings permits a lookback from 10 to 60 calendar days; swing research uses seven times that window. When both modes are enabled, separate intraday and swing comparisons retain the same selected stocks and membership snapshot, with explicit missing histories. Each mode has its own simulated account; this is not a combined intraday/swing portfolio backtest.
...
### Research settings and progress

The Research form saves only `research_symbols` and optional `research_cpu_affinity` through the authenticated, CSRF-protected `PUT /api/research/settings`. It rejects saves during active research or a pending restart without requiring paused trading. It does not start a job immediately. Parameter-count and refinement/application settings are no longer exposed or accepted, and `/api/research/apply` is removed. `research_workers: 0` means automatic comparison capacity; a positive value caps comparison workers. Old saved `research_tuning_workers` limits migrate to this setting in memory; other retired tuning settings are ignored.

`research_cpu_affinity` defaults to `pinned`; `automatic` allows operating-system scheduling. Windows and Linux workers verify their assigned logical CPU before reporting a successful pin. Unsupported/unavailable adapters fall back explicitly; failed pinning retires the worker and requires selecting automatic scheduling or resolving the reported restriction. The worker table reports actual assignments and states. Pinning does not reserve exclusive cores or guarantee measured utilization.

The single overall Research bar divides work across enabled intervals and spans collection, context and baseline/enhanced comparisons. It never moves backwards within a run, resets for a new run and reaches 100% on completion. Stage percentages and processed/total candles describe actual work, not elapsed time. The heading identifies the downloaded stock/timeframe, context preparation or comparison pass. The visible page polls every two seconds while active and five seconds otherwise, with one request at a time and polling stopped when hidden.
...
Sources: [historical-research.js](../src/historical-research.js), [research.js](../src/research.js), [backtest.js](../src/backtest.js).
