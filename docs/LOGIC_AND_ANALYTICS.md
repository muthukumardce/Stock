...
The browser and Cloudflare Tunnel provide access to the server. They do not perform the analysis. Closing the browser, signing out, or losing the tunnel connection does not pause the engine. Stopping the Python server stops local analysis and exit monitoring.

Sources: [trading.py](../app/trading.py), [main.py](../app/main.py), [broker.py](../app/broker.py).
...
| Paper capital | Rs 100,000 | Starting budget for the strategy simulation. |
| Live capital | Rs 0; live execution disabled | Real trading needs explicit server configuration. |
| Intraday | Enabled | Initially receives the configured mode's capital allocation. |
| Swing | Disabled; allocation Rs 0 | Overnight buying must be enabled and funded in Settings. |
...
| Daily loss threshold | 1% | Fraction of total configured bot capital, based on the bot's estimated daily P&L. |
...
Percent environment values are fractions: `0.0025` means 0.25%, not 0.0025%. Intraday and swing allocations together cannot exceed configured bot capital. Profit does not automatically increase that capital or enable swing. Settings changes are blocked while trading is running or managed exposure/orders remain.
...
Sources: [config.py](../app/config.py), `TradingEngine.connect`, `strategy_settings`, `_enter_locked` in [trading.py](../app/trading.py).
...
Sources: `CandleBook` in [strategy.py](../app/strategy.py); `_on_ticks`, `_history`, `_intraday_history`, `_seed_intraday` in [trading.py](../app/trading.py).
...
Source: `atr`, `intraday_signal`, `swing_signal` in [strategy.py](../app/strategy.py).
...
Automatic worker capacity is `max(1, logical_CPU_count - ANALYTICS_RESERVE_CPUS)`, with a default reserve of four. An explicit positive `ANALYTICS_WORKERS` sets a ceiling capped at the logical CPU count. On the detected 192-logical-CPU machine, the automatic ceiling is 188 workers.

Windows pools are split into shards of at most 61 workers. Processes start lazily; each task normally analyzes 32 instrument/strategy records. Runtime batch size is clamped to 1–256. A newer pending bar replaces an older queued bar for the same instrument and strategy, bounding the pending queue. Workers receive candle data, not broker credentials or database connections.
...
Source: [analytics.py](../app/analytics.py), `_analysis_loop` and `_analysis_finished` in [trading.py](../app/trading.py).
...
Remaining budget is limited by both the strategy's unused allocation and total configured capital, reduced for cumulative realized bot losses and current/pending exposure. Realized profits do not expand this budget automatically. Live cash is additionally bounded by reported cash and live balance after excluding collateral/adhoc margin and reserving bot notional exposure. The program reserves full purchase notional even if the broker would offer leverage.

The best ask's displayed quantity must cover the entire requested quantity; depth across multiple levels is not aggregated for this check. Combined planned stop distance risk for current bot positions, pending intraday entries and the proposed trade must not exceed `configured_capital × DAILY_LOSS_PCT`. This planned-distance calculation excludes fees and cannot cap actual losses during price gaps. Existing holdings are outside this bot-risk budget.

Sources: `position_size` in [strategy.py](../app/strategy.py); `_enter_locked`, `_exposure` in [trading.py](../app/trading.py).
...
New swing buys require enabled/funded swing settings and verified demat authorization. The implementation checks broker profile `meta.demat_consent == "physical"` for unattended DDPI/POA access; a local checkbox is not treated as proof.
...
For an explicit delivery exit, the engine first resolves earlier orders and disarms its GTT. It then verifies whether the GTT triggered during cancellation, accounts for resulting fills, refreshes holdings and sells only the remaining verified quantity using a CNC IOC limit order. An unknown cancellation, competing external order/GTT or unresolved prior exit prevents an additional sell. After a terminal partial exit, a later evaluation can request the remaining shares; it does not blindly repeat the original quantity.

Sources: [broker.py](../app/broker.py), `_exit_locked` and `_reconcile_live_locked` in [trading.py](../app/trading.py), [delivery.py](../app/delivery.py).
...
For live adoption, the engine requires suitable daily history, positive ATR, verified DDPI/POA, no competing external CNC order or unresolved GTT, and free settled CNC shares. Available quantity is:
...
Sources: `_analyze_holdings` in [trading.py](../app/trading.py); `_holding_available`, `_daily_bars`, `evaluate_holdings`, `request_exit` in [delivery.py](../app/delivery.py).
...
bot_equity = configured_capital + cumulative_bot_realised + bot_unrealised
...
When daily bot P&L is at or below `-configured_capital × DAILY_LOSS_PCT`, the engine stops new entries and requests exits for bot positions. Adopted pre-existing holdings are not part of this bot P&L or automatic daily-loss liquidation; their own exit rules and Close managed positions still apply. Price gaps, stale data and order failures can produce losses beyond the threshold.
...
Sources: `_unrealised`, `_daily_pnl`, `_roll_day`, `_sync_delivery`, `snapshot` in [trading.py](../app/trading.py); `snapshot`, `_realised` in [delivery.py](../app/delivery.py).
...
The SQLite database in `DATA_DIR` keeps bot journals, delivery intents, settings, account snapshots, historical cache, audit events and equity samples. It uses WAL mode and full synchronization. Order intentions are saved before external mutations. Losing this data loses the ownership/recovery journal even though real broker exposure can remain.

The activity log records signals and decision reasons, order state changes, executed trades, position/holding changes, sampled balance changes, history-loading progress, connection changes, settings changes, paper fills and errors. It does **not** persist every raw market tick or every rejected candle as a separate event. Candle rejection counts appear in approximately five-minute scan summaries. Balances are sampled at most once per minute; equity is sampled every 30 seconds while connected.
...
Sources: [storage.py](../app/storage.py), [main.py](../app/main.py), [trading.py](../app/trading.py), [delivery.py](../app/delivery.py).
